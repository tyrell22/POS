...
const PrinterSetup = ({ onBack, adminCode }) => {
    ...
    const loadPrinterConfiguration = async () => {
        try {
            ...
            if (response.ok) {
                ...
                setError(errorData.error || 'Грешка при вчитување на конфигурацијата');
            }
        } catch (err) {
    ...
    const loadAvailableOptions = async () => {
        try {
            // Load COM ports
            const portsResponse = await fetch('/api/printer/ports', {
            ...
            const printersResponse = await fetch('/api/printer/thermal-printers', {
                headers: { 'Admin-Code': adminCode }
            });
            ...
            if (connectionResponse.ok) {
                ...
                setConnectionTypes(connectionData.connectionTypes || []);
            }
        } catch (err) {
    ...
    const saveConfiguration = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Re-test connections after saving
                setTimeout(testPrinterConnections, 1000);
            } else {
                const errorData = await response.json();
