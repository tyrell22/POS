...
public class AreaService {
    ...
    public Area addTableToArea(Long areaId, Map<String, Object> tableData) {
        try {
            ...
            // Calculate grid position based on existing tables count
            int existingTablesCount = area.getTables() != null ? area.getTables().size() : 0;
    ...
    public void removeTableFromArea(Long areaId, Long tableId) {
        try {
            ...
            // Reorganize remaining tables in grid
            reorganizeTableGrid(area);
            
            logger.info("Removed table {} from area: {} and reorganized grid", table.getTableNumber(), area.getName());
    ...
    public void deleteTable(Long tableId) {
        try {
            ...
            logger.info("Deleted table {} and reorganized grid in area {}", table.getTableNumber(), area.getName());
    ...
    private void reorganizeTableGrid(Area area) {
        try {
            ...
            // Reorganize tables in 2-column grid
            ...
            tableRepository.flush();
            logger.info("Reorganized {} tables in grid for area {}", activeTables.size(), area.getName());
    ...
    // Standard area management methods remain the same
    ...
    public void deleteArea(Long id) {
        try {
            ...
            if (area.getTables() != null && !area.getTables().isEmpty()) {
                throw new RuntimeException("Не можете да ја избришете областа која содржи маси. Прво избришете ги масите.");
            }
